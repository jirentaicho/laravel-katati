...
→じゃあここでいうドメインバリデーションは何？
→例えば00個を超える場合は購入できないとか
→20個までしかアイテムを持てないなど



itemの値段がユーザーが持っている金額を超えているか？
...
class UserStock
  ...
  Stockが持ちきれない場合はダメ
  count Stock->where->user_id->count();
  count > 20 
    error

ドメインモデルがユースケースにて利用されます
